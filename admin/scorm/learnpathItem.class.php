...
class learnpathItem{
	...
	// vars to force the use of the scorm time
	var $force_scorm_time = false;
	var $time_before_session;
...
//	            '(@import([ \n\t\r]+)?("[^"]+"|\'[^\']+\'|[^ \n\t\r]+)))?/', -> takes a lot (like 100's of thousands of empty possibilities)
    ...
    function save($from_outside=true,$prereqs_complete=false)
    {
		...
		if($this->type == 'sco' || $this->type== 'au'){
			...
			if($this->prevent_reinit == 1 AND
			  ...
			  if($this->debug>1){error_log('New LP - In learnpathItem::save() - SCORM save request received'.$from_outside,0);}
    ...
    function set_status($status)
    {
    	...
    	// aicc compliance : get the complete value because only first letter is sent
		switch($status){
			case 'p' : $status = 'passed'; break;
			case 'f' : $status = 'failed'; break;
			case 'i' : $status = 'incomplete'; break;
			case 'n' : $status = 'not attempted'; break;
			case 'b' : $status = 'browsed'; break;
...
//		     					"AND order_id = $index";
     ...
     /**
      * Get the time spent on an item view (for this attempt)
      * @return the time spent on an item view in seconds
      */
     function get_db_time(){
     	
     	if(intval($this->attempt_id) == 0)
     		return 0;
     		
     	$tbl_iv = Database::get_course_table(TABLE_LP_ITEM_VIEW);
     	$sql = 'SELECT total_time FROM '.$tbl_iv.' WHERE id = '.intval($this->db_item_view_id);
     	$rs = Database::query($sql);
     	return Database::result($rs,0, 'total_time');
