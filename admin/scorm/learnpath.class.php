...
class learnpath {
     ...
     */
    function learnpath($lp_id, $user_id) {
        //check params
        //check course code
        if ($this->debug > 0) {
            error_log('New LP - In learnpath::learnpath(' . $lp_id . ',' . $user_id . ')', 0);
        }        
        //check learnpath ID
        if (empty($lp_id)) {
        ...
        if (empty($user_id)) {
            ...
            $sql = "SELECT * FROM $main_table WHERE id = '$user_id'";
            if ($this->debug > 2) {
    ...
    function get_next_item_id() {
        ...
        return 0;//return $this->ordered_items[$new_index]；修改2015 05 24
    }
     ...
     */
    static function get_package_type($file_path, $file_name) {
